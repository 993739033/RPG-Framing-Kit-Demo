...
public class UISaveSlotDisplayer : MonoBehaviour
{
    ...
    public void LoadSave()
    {
        ...
        //CanStreamedLevelBeLoaded 判断scene能否被加载
        if (string.IsNullOrEmpty(saveGame.lastScene) 
            || !Application.CanStreamedLevelBeLoaded(saveGame.lastScene))
        {
            confirmationWindow.Reference.
                GetComponent<ConfirmationWindow>().Configure(new ConfirmationWindow.Configuration()
            {
                acceptOnly = true,
