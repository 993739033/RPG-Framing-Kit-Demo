...
public class SaveSlotManager : MonoBehaviour
{
    ...
    private List<KeyValuePair<int, SaveGame>> saveGames; //keyValuePair只包含一个键值队
    ...
    {
        saveGames = SaveUtility.ObtainAllSaveGames()
            .OrderByDescending(save => (DateTime.Now - save.Value.saveDate))
            .ToList<KeyValuePair<int, SaveGame>>();

        if (saveGames.Count == 0)
        {
            confirmationWindow?.Reference?
                .GetComponent<ConfirmationWindow>()
                .Configure(new ConfirmationWindow.Configuration()
            {
                acceptOnly = true,
